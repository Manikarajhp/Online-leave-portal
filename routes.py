...
def register():
    ...
    if request.method == 'POST':
        ...
        # Ensure role is either 'employee' or 'manager'
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            # Only allow employees to login via this route
            if user.role != 'employee':
                flash('This login is for employees only. Managers should use the manager login page.', 'error')
                return redirect(url_for('login'))
...
def manager_login():
    if current_user.is_authenticated:
        # If already logged in as manager, go to manager dashboard; otherwise log out and continue
    ...
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            if user.role != 'manager':
                flash('This login is for managers only. Employees should use the regular login page.', 'error')
...
@login_required
def toggle_role():
    # Role switching is disabled. Managers and employees must use their respective logins.
    flash('Role switching is disabled. Please use the appropriate login page for managers or employees.', 'error')
    return redirect(url_for('dashboard'))
